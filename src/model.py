...
class UNet(object):
    def __init__(self, decodeImgShape=(320, 560, 1), outputShape=(320, 280, 1), numClasses=2,
                 dataPath=(None, None), batchSize=1, lr=1e-3, weightDecay=1e-4, totalIters=2e5, isTrain=True,
                 logDir=None, method=None, multi_test=True, advanced_multi_test=False, resize_factor=1.0,
                 use_dice_loss=False, use_batch_norm=False, lambda_one=1.0, name='UNet'):
    ...
    def _init_test_graph(self):
        ...
        # Batch for test data
        self.imgTests, _, self.img_name_test, self.user_id_test = testReader.batch(multi_test=self.multi_test,
                                                                                   use_advanced=self.advanced_multi_test)
        self.seg_GT = testReader.segImgOri
        ...
        if self.multi_test:
            ...
            shape = [1, *self.outputShape]
        self.imgTests = tf.reshape(self.imgTests, shape=shape)
    ...
    def set_conv_dims(self, method):
        ...
        if method == 'U-Net':
            ...
            conv_dims = [32, 32, 32, 32, 32, 32, 32, 32, 64, 64,
                         32, 32, 32, 32, 32, 32, 32, 32, 32, 16, 16, 16]
        elif method == 'U-Net-light-v4_3':
            conv_dims = [16, 32, 32, 32, 32, 32, 32, 32, 64, 64,
                         32, 32, 32, 32, 32, 32, 32, 32, 32, 16, 16, 16]
