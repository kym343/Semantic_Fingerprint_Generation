...
tf.flags.DEFINE_integer('iters', 2000000, 'number of iterations, default: 2,000,000')
...
tf.flags.DEFINE_integer('eval_freq', 1000, 'evaluation frequencey for evaluation of the batch accuracy, default: 100')
tf.flags.DEFINE_string('load_model', None, 'folder of saved model that you wish to continue training '
                                           '(e.g. 20200824-145423), default: None')
...
def test(solver, saver, modelDir, valDir, testDir, data):
    ...
    try:
        ...
        print("*" * 70)
        print(' Train')
        print(' - Best mIoU:      {:.3f}'.format(best_mIoU))
        print(' - Best Acc.:      {:.3f}'.format(best_acc))
        print(" - Best Precision: {:.3f}".format(best_precision))
        print(" - Best Recall:    {:.3f}".format(best_recall))
        print(" - Best F1 Score:  {:.3f}".format(best_f1_score))
        print("*" * 70)

        ################################################################################################################
        # mIoU, accuracy, precision, recall, f1_score = solver.test_test(save_dir=testDir)
