...
class Solver(object):
    ...
    def test_test(self, save_dir, is_debug=False, max_degree=10, interval=1, resize_factor=1.0):
        ...
        for iter_time in range(self.data.numTestImgs):
            ...
            s1_imgs, img_name, user_id, GT = self.sess.run([self.model.imgTests,
            ...
            print('[*] Stage 4: Calculaing the final prediction...')
            predCls = np.argmax(np.sum(s3_preds, axis=0, keepdims=True), axis=3)
            utils.save_imgs(img_stores=[np.expand_dims(s1_imgs[int(batch*0.5)], axis=0),
                                        np.expand_dims(np.argmax(s3_preds[int(batch*0.5)], axis=2), axis=0),
                                        predCls],
                            saveDir=os.path.join(save_dir, 'debug'),
                            img_name=img_name.astype('U26'),
                            is_vertical=False)
            ...
            name_append = '[Output]'
            cv2.imwrite(os.path.join(test_output, name_append + str(*img_name.astype('U26'))),
                        utils.convert_color_label(np.squeeze(predCls).astype(np.uint8)))

            name_append = '[GT]'
            cv2.imwrite(os.path.join(test_GT, name_append + str(*img_name.astype('U26'))),
                        utils.convert_color_label(np.squeeze(GT).astype(np.uint8)))
    ...
    def sample(self, iterTime, saveDir, num_imgs=2):
        feed = {
            self.model.ratePh: 0.5,  # rate: 1 - keep_prob
            self.model.trainMode: True
        }
